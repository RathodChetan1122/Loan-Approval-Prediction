...
import type { PredictionResponse, LoanApplication, ExplanationFactor, ActionPlanItem } from "../types/loan";
...
export default function PredictionResult({ result, initialApplication, onReset }: Props) {
  ...
  return (
    <section className={`result-page ${isApproved ? "result-approved" : "result-rejected"}`}>
          ...
          <section className="explainability-section" aria-labelledby="why-result-heading">
            <header className="explainability-header">
              ...
              <div>
                ...
                <p>The model identified a few factors that had the strongest influence on this assessment.</p>
              </div>
            </header>

            {/* Top Negative Contributing Factors — Collapsible Accordion */}
            <div className="accordion-group" role="region" aria-label="Negative contributing factors">
              {explanation.top_negative_factors.map((factor: ExplanationFactor, index: number) => {
                ...
                return (
                  <article className={`accordion-card negative-accordion ${isOpen ? "is-open" : ""}`} key={`${index}-${factor.feature}`}>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="accordion-trigger-right">
                        <span className={`impact-badge ${getImpactBadgeClass(factor.impact_level, factor.impact_direction)}`}>
                          {factor.impact_level}
                        </span>
              ...
              <div className="positive-factors-wrapper">
                ...
                <div className="accordion-group" role="region" aria-label="Positive factors">
                  {explanation.positive_factors.map((factor: ExplanationFactor, index: number) => {
                    ...
                    return (
                      <article className={`accordion-card positive-accordion ${isOpen ? "is-open" : ""}`} key={`pos-${index}-${factor.feature}`}>
                        <button
                          type="button"
                          className="accordion-trigger"
                        ...
                        >
                          <div className="accordion-trigger-left">
                            <span className="impact-dot" aria-hidden="true">🟢</span>
                            <span className="accordion-factor-label">{factor.label}</span>
                          </div>
            ...
            <section className="action-plan-section" aria-labelledby="action-plan-heading">
              <header className="action-plan-header">
                ...
                <div>
                  ...
                  <p>Here are the areas that may be worth working on based on the factors that influenced this assessment.</p>
                </div>
              </header>

              <div className="accordion-group action-accordion-group" role="region" aria-label="Personalized action plan items">
                {explanation.action_plan.map((item: ActionPlanItem, index: number) => {
                  ...
                  return (
                    <article className={`accordion-card action-accordion ${isOpen ? "is-open" : ""}`} key={`action-${item.priority}`}>
                      <button
                        type="button"
          ...
          {explanation && explanation.positive_factors.length > 0 && (
            <div className="accordion-group" style={{ marginBottom: "18px" }}>
              {explanation.positive_factors.map((factor: ExplanationFactor, index: number) => {
                ...
                return (
                  <article className={`accordion-card positive-accordion ${isOpen ? "is-open" : ""}`} key={`app-pos-${index}-${factor.feature}`}>
                    <button
                      type="button"
                      className="accordion-trigger"
      ...
      {/* 5. End of Flow: Download Assessment Report + Check Another Application */}
